...
function BatterBowlTypeAnalysis({ player }) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/batter-bowl-types?player=${encodeURIComponent(player)}`)
      ...
      .catch(error => {
        console.error('Error fetching bowl type analysis:', error);
        setError('Failed to fetch bowl type analysis data');
        setLoading(false);
      });
  }, [player]);
                ...
                >
                  <td className="px-6 py-4">
                    <span className="font-medium text-gray-900">
                      {formatShotName(shot.bowl_style)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <ControlBar value={parseFloat(shot.dot_pct)} />
                  </td>
                  <td className="px-6 py-4">
                    <ControlBar value={parseFloat(shot.boundary_pct)} />
                  </td>
                </tr>
              ))}
            </tbody>
...
export default BatterBowlTypeAnalysis;