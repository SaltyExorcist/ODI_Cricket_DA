...
const PitchHeatmap3D = ({ player }) => {
  ...
  const getWickets = (line, length) => {
    const match = data.find((d) => d.line === line && d.length === length);
    return match ? parseFloat(match.wickets) : null;
  };
        ...
        <div className="relative w-[85%] h-[85%]"
          ...
          >
            {lengthOrder.map((length) =>
              lineOrder.map((line) => {
                const wickets = getWickets(line, length);
                return (
                  <div
                    key={`${length}-${line}`}
                    title={`${line} | ${length}: Wickets ${wickets ? wickets.toFixed(1) : "–"}`}
                    style={{
                      backgroundColor: getColor(wickets),
